...
# --- OPTIMIZED: Physics process removed to prevent double raycasting ---
# The LevelLightManager now controls the beam's raycasting and logic.
# This script is now responsible for VISUALS and HIT RESPONSE only.

func update_beam_visuals(end_point_local: Vector2, is_colliding_with_something: bool, collision_normal: Vector2) -> void:
    # Update Line2D
    line_2d.points[1] = end_point_local
    
    # Update Beam Particles (The glow along the beam)
    if beam_particles:
        var laser_start_position := line_2d.points[0]
        beam_particles.position = laser_start_position + (end_point_local - laser_start_position) * 0.5
        var material = beam_particles.process_material
        if material is ParticleProcessMaterial:
            material.emission_box_extents.x = end_point_local.distance_to(laser_start_position) * 0.5
    
    # Update Collision Particles (The sparks at the end)
    if collision_particles:
        collision_particles.emitting = is_colliding_with_something
        if is_colliding_with_something:
            collision_particles.position = end_point_local
            collision_particles.global_rotation = collision_normal.angle()

func handle_hit_logic(collider: Object) -> void:
